...
from pyflink.datastream import StreamExecutionEnvironment, CheckpointingMode, CheckpointStorage
from pyflink.datastream.checkpoint_config import ExternalizedCheckpointCleanup
from pyflink.datastream.connectors.kafka import FlinkKafkaConsumer, FlinkKafkaProducer
from pyflink.datastream.functions import CoProcessFunction, KeyedProcessFunction, RuntimeContext
...
class MergeReviewsWithMetadata(CoProcessFunction):
    def open(self, ctx: RuntimeContext):
        self.metadata_state = ctx.get_state(ValueStateDescriptor("metadata", Types.STRING()))
        self.reviews_state = ctx.get_list_state(ListStateDescriptor("reviews", Types.STRING()))
    ...
    def process_element1(self, value, ctx: CoProcessFunction.Context):
        ...
        metadata = json.loads(self.metadata_state.value())["after"] if self.metadata_state.value() else None
            ...
            for field in self.review_schema["fields"]:
                for key in field.keys():
                    if key != "name":
                        continue
                    name = field[key]
                    if name == "product_id": merged_record[name] = review["asin"]
                    elif name == "review_title": merged_record[name] = review["title"]
                    elif name == "review_text": merged_record[name] = review["text"]
                    elif name == "parent_product_id": merged_record[name] = review["parent_asin"]
                    else: merged_record[name] = review[name]
            ...
            for field in self.metadata_schema["fields"]:
                for key in field.keys():
                    if key != "name":
                        continue
                    name = field[key]
                    if name == "parent_product_id": continue
                    elif name == "product_title": merged_record[name] = metadata["title"]
                    elif name == "category": merged_record[name] = metadata["main_category"]
                    else: merged_record[name] = metadata[name]
            yield merged_record
        else:
...
def main():
    ...
    enriched_stream = connected_stream.process(MergeReviewsWithMetadata(), output_type=Types.PICKLED_BYTE_ARRAY())
    ...
    enriched_stream \
    .map(process_features, output_type=Types.STRING()) \
    .filter(lambda x: x is not None) \
    .add_sink(producer) \
    .name("Sink merged streams")
