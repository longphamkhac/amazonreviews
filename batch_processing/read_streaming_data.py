...
def main():
    ...
    dataframe_schema = avro_to_dataframe_schema(
        json.loads(get_schema(MERGE_SCHEMA_TOPIC)), mapping
    )

    deserializer = AvroDeserializerWrapper(
        schema_registry_conf=SCHEMA_REGISTRY_CONF, topic=MERGE_SCHEMA_TOPIC
    )
    ...
    def process(batch_df, batch_id):
        ...
        def process_raw(row):
            ...
            raw_dict["timestamp"] = datetime.strptime(
                raw_dict["timestamp"], "%Y-%m-%d %H:%M:%S"
            )
            return raw_dict
        ...
        if not transformed_rdd.isEmpty():
            ...
            )  # insert review_id column as UUIDv7
            transformed_df = transformed_df.withColumn(
                "time_id", uuidv7_udf()
            )  # insert time_id column as uuidv7
            transformed_df.write.format("delta").mode("append").save(
                f"s3a://{BUCKET_NAME}/{STREAMING_DATA_FOLDER}"
            )
            try:
                transformed_df.write.format("delta").mode("append").save(
    ...
    kafka_df.writeStream.foreachBatch(process).option(
        "checkpointLocation", f"s3a://{BUCKET_NAME}/checkpoints/{STREAMING_DATA_FOLDER}"
    ).trigger(processingTime=f"{BATCH_PROCESSING_TIME}").start().awaitTermination()
