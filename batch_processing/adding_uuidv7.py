...
def main():
    ...
    assert isFound
    df_merge = spark.read.format("delta").load(f"s3a://{BUCKET_NAME}/{MERGE_DATA_FOLDER}")
    df_merge = df_merge.withColumn("review_id", uuidv7_udf()) # adding review_id as uuidv7
    ...
    df_merge.write.format("delta").mode("append").save(f"s3a://{BUCKET_NAME}/{REFINED_MERGE_DATA_FOLDER}/")
